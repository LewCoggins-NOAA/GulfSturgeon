...
#  Madult <- parameters$Madult   # vector of age-specific survival (after age at recruitment) [length=A-AR+1]
  ...
  V1 <- No           # initial vulnerable abundance (used to create initial population)
...
results=matrix(response,numM,numNoVals)
res.df=as.data.frame(cbind(M,numNoVals,response))
obj<- list(x=M[1:numM], y=matrix(No,numM,numNoVals)[1,],z=results)
...
fig1=plot_ly(z=~ProbabilityExtripation100Years,y=~Mortality,x=~InitialAbundance)
fig1 <- fig1 %>% add_surface()
fig1

fig2=plot_ly(z=~ProbabilityExtripation100Years,y=~Mortality,x=~InitialAbundance,type="contour")
fig2
